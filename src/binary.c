...
static _i32 _malelf_binary_map_shdr(MalelfBinary *bin)
{
	...
	case MALELF_ELFNONE: 
		...
		break;
	case MALELF_ELF32: 
		bin->shdr.h32 = (Elf32_Shdr *) (bin->mem + ehdr.uhdr.h32->e_shoff);
		break;
	case MALELF_ELF64: 
		bin->shdr.h64 = (Elf64_Shdr *) (bin->mem + ehdr.uhdr.h64->e_shoff);
		break;
	}
	return MALELF_SUCCESS;
}
					     ...
					     MalelfBinary *bin)
{
	...
	error = malelf_binary_get_shdr(bin, &uhdr);
	...
	case MALELF_ELF32: {
		...
		return (char *)(bin->mem + 
			bin->shdr.h32[bin->ehdr.uhdr.h32->e_shstrndx].sh_offset + 
			shdr32->sh_name);
	}
	case MALELF_ELF64: {
		shdr64 = uhdr.h64;
		...
		return (char *)(bin->mem + 
			bin->shdr.h64[bin->ehdr.uhdr.h64->e_shstrndx].sh_offset + 
			shdr64->sh_name);
	}
	default:
		return NULL;
